...
import com.weibo.martn.weiboapp.adapter.CommentListAdapter;
...
public class FragmentComment extends FragmentBase {
	...
	private void initToken() {
		...
		// 获取微博评论信息接口
		mCommentsAPI = new CommentsAPI(getActivity(), Constants.SINA_APP_KEY, mAccessToken);
				...
				if (comments != null && comments.total_number > 0) {
					...
					if (swingBottomInAnimationAdapter == null)
						swingBottomInAnimationAdapter = new SwingBottomInAnimationAdapter(
								mCommentListAdapter);
					swingBottomInAnimationAdapter.setAbsListView(mCommentListView);
					// anmi
					mCommentListView.setAdapter(mCommentListAdapter);
					// save data
